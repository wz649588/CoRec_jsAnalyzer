...
public class ChangeFact {
	...
	public ChangeFact (File leftJs, File rightJs, TraverseJsonTyped tJl,TraverseJsonTyped tJr){
		this.leftJs = leftJs;
	...
	public void buildDeletedFact() {
		for(String leftVar : tJl.varSet) {
			deletedVars.add(new ClientVar(leftVar, tJl.nameToITree.get(leftVar),  tJl.typedNameToITree.get(leftVar), tJl.jsFilePath, tJl));
		}
		for(String leftFunc : tJl.funcSet) {
			deletedFuncs.add(new ClientFunc(leftFunc, tJl.nameToITree.get(leftFunc), tJl.typedNameToITree.get(leftFunc), tJl.jsFilePath, tJl));
		}
		for(String leftClass : tJl.classSet) {
			deletedClasses.add(new ClientClass(leftClass, tJl.nameToITree.get(leftClass), tJl.typedNameToITree.get(leftClass), tJl.jsFilePath, tJl));
		}
		for(String leftExp : tJl.expSet) {
			deletedExps.add(new ClientExp(leftExp, tJl.nameToITree.get(leftExp), tJl.typedNameToITree.get(leftExp), tJl.jsFilePath, tJl));
		}
	}
	
	public void buildAddedFact() {
		for(String rightVar : tJr.varSet) {
			insertedVars.add(new ClientVar(rightVar, tJr.nameToITree.get(rightVar), tJr.typedNameToITree.get(rightVar), tJr.jsFilePath, tJr));
		}
		for(String rightFunc : tJr.funcSet) {
			insertedFuncs.add(new ClientFunc(rightFunc, tJr.nameToITree.get(rightFunc), tJr.typedNameToITree.get(rightFunc), tJr.jsFilePath, tJr));
		}
		for(String rightClass : tJr.classSet) {
			insertedClasses.add(new ClientClass(rightClass, tJr.nameToITree.get(rightClass), tJr.typedNameToITree.get(rightClass), tJr.jsFilePath, tJr));
		}
		for(String rightExp : tJr.expSet) {
			insertedExps.add(new ClientExp(rightExp, tJr.nameToITree.get(rightExp), tJr.typedNameToITree.get(rightExp), tJr.jsFilePath, tJr));
		}
	}
...
//		variables
		for(String leftVar : tJl.varSet){
			if(!tJr.varSet.contains(leftVar)) deletedVars.add(new ClientVar(leftVar, tJl.nameToITree.get(leftVar), tJl.typedNameToITree.get(leftVar), tJl.jsFilePath, tJl));
			else{
//				System.out.println("Does it take long time?");
				...
				ITree typedLVarTree = tJl.typedNameToITree.get(leftVar), typedRVarTree = tJr.typedNameToITree.get(leftVar);
				if(!lVarToken.equals(rVarToken)) {
					changedVars.add(new Pair(new ClientVar(leftVar, lVarTree, typedLVarTree, tJl.jsFilePath, tJl), new ClientVar
							(leftVar, rVarTree, typedRVarTree, tJr.jsFilePath, tJr)));
				}
//				System.out.println("Does it take long time?");
//				ITree lVarTree = tJl.nameToITree.get(leftVar), rVarTree = tJr.nameToITree.get(leftVar);
...
//				}
		...
		for(String rightVar : tJr.varSet){
			if(!tJl.varSet.contains(rightVar)) insertedVars.add(new ClientVar(rightVar, tJr.nameToITree.get(rightVar), tJr.typedNameToITree.get(rightVar),tJr.jsFilePath, tJr));
		}
		
//		functions
		for(String leftFunc : tJl.funcSet){
			if(!tJr.funcSet.contains(leftFunc)) deletedFuncs.add(new ClientFunc(leftFunc, tJl.nameToITree.get(leftFunc), tJl.typedNameToITree.get(leftFunc), tJl.jsFilePath, tJl));
			else{
//				System.out.println("Does it take long time?");
				...
				ITree typedLFuncTree = tJl.typedNameToITree.get(leftFunc), typedRFuncTree = tJr.typedNameToITree.get(leftFunc);
				if(!lFuncToken.equals(rFuncToken)) {
					changedFuncs.add(new Pair(new ClientFunc(leftFunc, lFuncTree,typedLFuncTree, tJl.jsFilePath, tJl), new ClientFunc
							(leftFunc, rFuncTree,typedRFuncTree, tJr.jsFilePath, tJr)));
				}
//				ITree lFuncTree = tJl.nameToITree.get(leftFunc), rFuncTree = tJr.nameToITree.get(leftFunc);
...
//				}
		...
		for(String rightFunc : tJr.funcSet){
			if(!tJl.funcSet.contains(rightFunc)) insertedFuncs.add(new ClientFunc(rightFunc, tJr.nameToITree.get(rightFunc), tJr.typedNameToITree.get(rightFunc), tJr.jsFilePath, tJr));
		}
		
//		classes
		/*
		 * change of a class can only mean the extension has changed
		 */
		for(String leftClass : tJl.classSet){
			if(!tJr.classSet.contains(leftClass)) deletedClasses.add(new ClientClass(leftClass, tJl.nameToITree.get(leftClass), tJl.typedNameToITree.get(leftClass), tJl.jsFilePath, tJl));
			else{
//				System.out.println("Does it take long time?");
...
//				}
		...
		for(String rightClass : tJr.classSet){
			if(!tJl.classSet.contains(rightClass)) insertedClasses.add(new ClientClass(rightClass, tJr.nameToITree.get(rightClass), tJr.typedNameToITree.get(rightClass), tJr.jsFilePath, tJr));
		}
...
//		}
		...
		PriorityQueue<MatchTuple> expMatchings = new PriorityQueue<>();
...
//		Build the expmatching priorityqueue
		for(String leftExp : tJl.expSet){
			...
			ITree lExpTree = tJl.nameToITree.get(leftExp);
			ITree typedLExpTree = tJl.typedNameToITree.get(leftExp);
			...
			for(String rightExp : tJr.expSet){
				...
				if(rightExp.startsWith(expKind)){
					ITree rExpTree = tJr.nameToITree.get(rightExp);
					ITree typedRExpTree = tJr.typedNameToITree.get(rightExp);
...
//					System.out.println(match);
					...
					if(match >= 0.5){
						expMatchings.offer(new MatchTuple(match, leftExp, rightExp, lExpTree, rExpTree, typedLExpTree, typedRExpTree));
						tempChange = rightExp;
					}
				}
			}
...
//		changed expressions
		while(!expMatchings.isEmpty()){
			MatchTuple expMatch = expMatchings.poll();
			if(visitedLeftExp.contains(expMatch.leftExp) || visitedRightExp.contains(expMatch.rightExp)) continue;
			changedExps.add(new Pair(new ClientExp(expMatch.leftExp, expMatch.lExpTree, expMatch.typedLExpTree, tJl.jsFilePath, tJl), new ClientExp
					(expMatch.rightExp, expMatch.rExpTree, expMatch.typedRExpTree, tJr.jsFilePath, tJr)));
...
//		deleted expressions
		for(String leftExp : tJl.expSet){
			if(!visitedLeftExp.contains(leftExp)){
				ITree lExpTree = tJl.nameToITree.get(leftExp);
				ITree typedLExpTree = tJl.typedNameToITree.get(leftExp);
				deletedExps.add(new ClientExp(leftExp, lExpTree, typedLExpTree, tJl.jsFilePath, tJl));
				visitedLeftExp.add(leftExp);
			}
		}
...
//		added expressions
		for(String rightExp : tJr.expSet){
			if(!visitedRightExp.contains(rightExp)){
				ITree rExpTree = tJr.nameToITree.get(rightExp);
				ITree typedRExpTree = tJr.typedNameToITree.get(rightExp);
				insertedExps.add(new ClientExp(rightExp, rExpTree, typedRExpTree, tJr.jsFilePath, tJr));
			}
		}
...
//	}
    ...
    public static void extractChangesForRose(String commitName, List<ChangeFact> cfList) throws SQLException {
...
class MatchTuple implements Comparable<MatchTuple>{
	double match;
	String leftExp, rightExp;
	ITree lExpTree, rExpTree, typedLExpTree, typedRExpTree;
	public MatchTuple(double match, String leftExp, String rightExp, ITree lExpTree, ITree rExpTree, ITree typedLExpTree, ITree typedRExpTree){
		this.match = match;
		this.leftExp = leftExp;
		this.rightExp = rightExp;
		this.lExpTree = lExpTree;
		this.rExpTree = rExpTree;
		this.typedLExpTree = typedLExpTree;
		this.typedRExpTree = typedRExpTree;
	}
	@Override
	public int compareTo(MatchTuple mT){
